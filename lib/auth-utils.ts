...
export function isTokenExpired(token: string): boolean {
    try {
        ...
        logger.error('Error parsing token for expiration check:', error)
...
export async function getValidToken(): Promise<string | null> {
    try {
        ...
        if (!token) {
            logger.warn('getValidToken: No token available')
            return null
        }

        if (isTokenExpired(token)) {
            logger.warn('getValidToken: Token is expired, triggering refresh')
        ...
        logger.error('getValidToken: Error getting token:', error)

        if (isAuthError(error)) {
            logger.info('getValidToken: Auth error detected, triggering page reload')
            window.location.reload()
        }
...
export function handleApiError(error: unknown, operation: string): never {
    ...
    if (isAuthError(error)) {
        logger.info(`Auth error detected in ${operation}, triggering page reload`)
        window.location.reload()
    }
...
if (typeof window !== 'undefined') {
    ...
    (window as any).isAuthError = isAuthError;
    (window as any).getValidToken = getValidToken;
    (window as any).handleApiError = handleApiError;
    (window as any).withAuthErrorHandling = withAuthErrorHandling;
}
